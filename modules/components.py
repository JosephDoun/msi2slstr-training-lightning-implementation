...
class BGNorm(nn.BatchNorm2d):
    def __init__(self, _in: int, groups: int, *args, **kwargs) -> None:
        super().__init__(num_features=groups, *args, **kwargs)
        assert not _in % groups, "in channels not divisible by groups."
        self._groups = groups

    def forward(self, x: Tensor):
        shape = x.shape
        x = x.view(shape[0], self._groups,
                   shape[1] // self._groups,
                   shape[2] *
                   shape[3]).contiguous()
        return super().forward(x).view(shape).contiguous()
